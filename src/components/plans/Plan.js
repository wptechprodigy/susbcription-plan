...
function Plan({ subscriptionPlan }) {
  const { price, name, color, description, features } = subscriptionPlan;
	...
	return (
		<>
			<div
				className={`flex flex-col justify-center align-middle ${color} w-full pt-10 pb-4 pl-10 pr-10 overflow-hidden rounded`}>
				<p className='text-center font-semibold text-2xl'>{name}</p>
				<h3 className='font-bold text-center'>
					<span className='inline-block align-top pt-2 text-2xl font-semibold'>
						$
					</span>
					<span className='text-4xl'>{price}</span>
				</h3>
				<p className='text-center font-medium text-white my-5'>{description}</p>
